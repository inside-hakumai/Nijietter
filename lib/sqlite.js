...
MeCab.command = "mecab -d " + process.env.MECAB_NEOLOGD_LOCATION;
let mecab = new MeCab();
...
class Database {
   ...
   insert_image(mediaID, slackFileID, slackPostTs, userID) {
      ...
      // noinspection JSUnresolvedFunction
      this.database.serialize(function () {
         let stmt = parentThis.database.prepare('INSERT OR REPLACE INTO image(media_id, slack_file_id, slack_post_ts, user_id) '
            + 'VALUES(?, ?, ?, ?)');
   ...
   insert_tweet(mediaID, text) {
      ...
      // noinspection JSUnresolvedFunction
      this.database.serialize(function () {
         let stmt = parentThis.database.prepare('INSERT OR REPLACE INTO tweet(media_id, tweet_text) '
            + 'VALUES(?, ?)');
   ...
   update_nijie_bool_from_mediaid(media_id, is_nijie){
      let parentThis = this;
      // noinspection JSUnresolvedFunction
      this.database.serialize(function () {
         logger_db.raiseHierLevel('debug');
         logger_db.debug(`INSERT OR REPLACE nijie_bool - ${media_id}, ${is_nijie}`);
         let stmt = parentThis.database.prepare('INSERT OR REPLACE INTO nijie_bool(media_id, is_nijie) '
            + 'VALUES(?, ?)');
         stmt.run([media_id, is_nijie]);
         // noinspection JSUnresolvedFunction
         stmt.finalize();
         logger_db.dropHierLevel('debug');
      });
