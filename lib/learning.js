...
class LModel {
   ...
   prepare_vocabulary() {
      ...
      console.log("Preparing vocabulary...");
      ...
      let word_prep = parentThis.db.get_all_word_appearance().then(function(result){
         return new Promise(function(resolve) {
            ...
            console.log(`#Word appearance:           ${words.length}`);

            let word_count = 0;
            let words_unique = [];
            let word_counter = appearance_count(words);
            console.log(`#Unique words:              ${Object.keys(word_counter).length}`);
            ...
            console.log(`#Excluded unique words:     ${excluded_words}`);
            console.log(`#Remaining unique words:    ${words_unique.length}`);
            console.log(`#Remaining word appearance: ${word_count}`);
      ...
      let user_prep = parentThis.db.get_all_user_appearance().then(function(result){
         return new Promise(function(resolve) {
            ...
            console.log(`#User appearance:           ${users.length}`);

            let user_count = 0;
            let users_unique = [];
            let user_counter = appearance_count(users);
            console.log(`#Unique users:              ${user_counter.length}`);
            ...
            console.log(`#Excluded unique users:     ${excluded_users}`);
            console.log(`#Remaining unique users:    ${Object.keys(users_unique).length}`);
            console.log(`#Remaining user appearance: ${user_count}`);
   ...
   predict(username, tweet) {
      ...
      return this.estimator.predictSync([user_score, tweet_score]);
   }
...
function write_json(file_name, data){
   ...
   console.log(`Log saved: ${file_path}`);
}
